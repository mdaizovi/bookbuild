...
from .models import (Author, Contributor,
...
class RecipeAdmin(OrderedModelAdmin):
    ...
    list_display= ('title', 'permission', 'joy_edited', "has_author_questions", 'contributor', 'publish','move_up_down_links','order','halfpage')
    ...
    fields = ("title", "book", 
            ...
            "ingredients_html", 
            "ingredients_html2",
            "steps_html", "intro", "author_questions")
    ...
    def toggle_published(self, request, queryset):
        if request.POST.get('action') == 'toggle_published':
            for obj in queryset:
                ...
                obj.save()
            self.message_user(request, "%s chapters successfully changed." % queryset.count()) 
            return
#===============================================================================
