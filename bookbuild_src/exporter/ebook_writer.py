...
class EbookWriter:
    def __init__(self, book=None, get_assets=False):
        ...
        for f in self.book.files.all():
            self.CSS_SNIPPET += '<link href="%s" rel="stylesheet" type="text/css"/>' % f.relative_url
        #self.CSS_SNIPPET = '<link href="css/stylesheet.css" rel="stylesheet" type="text/css"/>'
        
        self.get_assets = get_assets # whether or not to download images, css, etc from aws
        
        self.BOOK_BASE_DIR = os.path.join(settings.BASE_DIR, 'exporter')
        self.DESTINATION_MEDIA_PATH = os.path.join(self.BOOK_BASE_DIR, 'Add2Epub', 'OEBPS', 'media')
        ...
        for file in my_bucket.objects.filter(Prefix=s3_root_folder_prefix):
            if any(s in file.key for s in s3_folder_list):
                try:
                    ...
                    s3_resource.meta.client.download_file(settings.AWS_STORAGE_BUCKET_NAME, file.key, full_img_path)
    ...
    def writeComponent(self, component_type, chapter=None):
        ...
        html_path_list = [settings.BASE_DIR, 'templates', 'exporter', self.book.book_type, component_type + 'Base.html']
        
        if component_type == "cover":
            html_destination = os.path.join(self.BOOK_BASE_DIR, 'Add2Epub', "OEBPS/001_cover.html")
        elif component_type == "chapter" and chapter:
            if hasattr(chapter, "section"):
                html_path_list = [settings.BASE_DIR, 'templates', 'exporter', self.book.book_type, 'sectionBase.html']
                ctx.update({
                        "section" : chapter.section,
                        "subsections" : chapter.section.subsection_set.filter(publish = True).order_by('order')
                        })
            else:
            ...
            chapters_all = self.book.chapter_set.filter(publish=True).order_by("playOrder").exclude(title__in = exclude) 
            chapters = OrderedDict()
            for c in chapters_all:
                if hasattr(c, "section"):
                    chapters.update({c:list(c.section.subsection_set.filter(publish = True).order_by('order'))})
                else:
                    chapters.update({c: []})
            ...
            if len(contributers) > 0:
                ctx.update({
                        ...
                        "sections": Section.objects.all().order_by("chapter__playOrder"),
                })
    ...
    def writeOPF(self):
        ...
        opf_str += "<metadata>"
        for attr in ["title", "identifier", "creator", "publisher", "date", 
                "language", "subject", "description", "format", "type", "rights"
                ]:
            opf_str += ("<dc:" + attr + ">" + str(self.metaDataDict.get(attr)) + "</dc:" + attr + ">")
        ...
        for dirname, media_type in {"css": "text/css", "fonts": 'application/vnd.ms-opentype', "images": "image/jpeg" }.items():
            this_index = 0
            this_dir = os.path.join(self.BOOK_BASE_DIR, 'Add2Epub','OEBPS', dirname)
            for f in os.listdir(this_dir):
                this_index += 1
                opf_str += ("<item href='OEBPS/" + dirname + "/" + f + "' id = '" + dirname[0] + str(this_index) + "' media-type='" + media_type + "'/>")

        if self.book.cover:
            cover_filename = "media/" + self.book.cover.get_file_name()
        else:
            cover_filename = "images/cover.jpg"
        opf_str += ("<item href='OEBPS/" + cover_filename + "' id = 'cover-image' media-type='image/jpeg'/>")
        ...
        for c in Chapter.objects.filter(book=self.book, publish=True):
            ...
            opf_str += ("<item href='" + str(c.src) + "' id='" + html_id + "' media-type='application/xhtml+xml'/>")
    ...
    def writeTOC(self):
        ...
        for c in list(self.book.chapter_set.filter(publish=True).order_by("playOrder")):
            toc_str += ("<navPoint id='navPoint-" + str(c.playOrder) + "' playOrder='" + str(c.playOrder) + "'>")
    ...
    def buildNewEpub(self, thisTitle = "newbook"):
        ...
        if os.path.exists(book_dir):
            ...
            os.rename(os.path.join(self.BOOK_BASE_DIR, (thisTitle + ".zip")), os.path.join(self.BOOK_BASE_DIR, (thisTitle + ".epub")))
    ...
    def writeBook(self):
        ...
        if len(imageList) > 1:
            ...
            for i in os.listdir(settings.MEDIA_ROOT + "/img/"):
                copyanything(settings.MEDIA_ROOT + "/img/" + i, self.DESTINATION_MEDIA_PATH)

        filelist = self.book.files.all()
        for f in filelist:
            DESTINATION_FILE_PATH = os.path.join(self.BOOK_BASE_DIR, 'Add2Epub', 'OEBPS', f.file_type)
            if not os.path.exists(DESTINATION_FILE_PATH):
                os.makedirs(DESTINATION_FILE_PATH)
            local_location = os.path.join(settings.MEDIA_ROOT, 'exporter', 'static', f.filename)
            copyanything(local_location, DESTINATION_FILE_PATH)

        self.writeComponent("cover")
        for c in self.book.chapter_set.filter(publish=True).order_by("playOrder").exclude(title__in=["Contents", "Contributors"]):
