...
def download_images_from_aws(verbose=False):
    ...
    print("About to start downloading images") if verbose else None
    for file in my_bucket.objects.filter(Prefix=s3_root_folder_prefix):
        if any(s in file.key for s in s3_folder_list):
            try:
                ...
                print(file.key, " downloaded ") if verbose else None
            except Exception as err:
                print("exception downloading file: ", err)
    print("Done downloading images") if verbose else None


def download_static_from_aws(verbose=False):
    ...
    print("About to start downloading static") if verbose else None
    for file in my_bucket.objects.filter(Prefix=s3_root_folder_prefix):
        if any(s in file.key for s in s3_folder_list):
            try:
                path, filename = os.path.split(file.key)
                ...
                print("exception downloading file: ", err)
    print("Done downloading static") if verbose else None
