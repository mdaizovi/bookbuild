    ...
    def handle(self, *args, **options):
        ...
        for n in Neighborhood.objects.order_by("order"):
            ...
            for b in Blob.objects.filter(neighborhood=n):
                if b.category != None and b.category.title == "Kapitelbeschreibung":
            ...
            if n.title == "Ausflug":
                ...
                for c in Section.objects.order_by("order"):
                    if c.title in [
                        ...
                        "Ganztagesausflug",
                        "Mehrtagesausflug",
                    ]:  # and Blob.objects.filter(neighborhood=n, priority__gte=2, priority__lte=4).filter(section=c).order_by('-priority').count()>0:

                        document.add_heading(c.title, level=1)

                        # walk through blobs in section Ausflug
                        for b in Blob.objects.filter(
                            neighborhood=n, priority__gte=2, section=c
                        ).order_by("-priority"):
            ...
            for b in Blob.objects.filter(neighborhood=n, priority__gte=5).order_by(
            ...
            for c in Section.objects.order_by("order"):
                ...
                if (
                    Blob.objects.filter(
                        neighborhood=n, priority__gte=2, priority__lte=4
                    )
                    .filter(category__section=c)
                    .count()
                    > 0
                ):
        ...
        filepath_booklet = os.path.join(
            settings.BASE_DIR, "importer", "export", filename_booklet
        )
        ...
        for n in Neighborhood.objects.order_by("order"):
            ...
            for c in Section.objects.order_by("order"):
                if c.title != "Routen":
                    continue
                for b in (
                    Blob.objects.filter(neighborhood=n, priority__gte=2)
                    .filter(category__section=c)
                    .order_by("-priority")
                ):
        ...
        filepath_header = os.path.join(
            settings.BASE_DIR, "importer", "export", filename_header
        )
        ...
        for n in Neighborhood.objects.order_by("order"):
            ...
            for c in Section.objects.order_by("order"):
                if (
                    Blob.objects.filter(
                        neighborhood=n, priority__gte=2, priority__lte=5
                    )
                    .filter(category__section=c)
                    .count()
                    > 0
